...
const MarkerSelectModal = ({
  ...
  const { data } = useQuery<MarkerSelectModalQuery, MarkerSelectModalQueryVariables>(MARKER_SELECT_MODAL_QUERY, {
    variables: {
    ...
    try {
      uploadMarker({
        ...
        update: (cache, { data }) => {
          ...
          const existingMarkers = cache.readQuery<MarkerSelectModalQuery, MarkerSelectModalQueryVariables>({ query: MARKER_SELECT_MODAL_QUERY, variables: { userId: currentUser?.id } })
          if (existingMarkers && newMarker) {
            cache.writeQuery<MarkerSelectModalQuery, MarkerSelectModalQueryVariables>({
              query: MARKER_SELECT_MODAL_QUERY,
              variables: { userId: currentUser?.id },
              data: {
                vendorMarkers: [...existingMarkers.vendorMarkers, newMarker]
              }
            })
          }
        }
    ...
    >
      ...
      >
        ...
        { data?.vendorMarkers.map((marker) => (
          ...
          >
            <img src={marker.url} alt="marker icon" className='object-scale-down'/>
          </div>
        ))}
        <Button
          disabled={uploadMarkerLoading || data?.vendorMarkers.length >= 2}
          onClick={() => {
        ...
        <CustomMarkerUploadModal
          ...
          onClose={() => { setIsUploadCustomMarkerModalOpen(false)} }
          onSubmit={(url) => {
      ...
      <p className="mt-8 text-sm">
        Note: You can only have up to 3 custom markers. If you want to add a new one, delete existing markers on your profile page.
      </p>
    </BaseModal>
  )
}
