...
const MapPage = () => {
    ...
    const { data: vendorProductsQueryData } = useQuery<VendorProductsQuery>(
        VENDOR_PRODUCTS_QUERY
    )
    const [products, setProducts] = useState<
        VendorProductsQuery['vendorProducts']
    >([])

    const [selectedVendor, setSelectedVendor] =
        useState<MapVendorsQuery['mapVendors'][number]>(null)
    ...
    useEffect(() => {
        if (pusher && channel) {
            ...
            channel.bind(
                'image-delete',
                ({ deletedImage }: { deletedImage: Image }) => {
                    apolloClient.cache.updateQuery(
                        { query: MAP_VENDORS_QUERY },
                        (data) => ({
                            mapVendors: data.mapVendors.map((vendor) => {
    ...
    const searchProductHandler = (selectedProduct: {
        value: number
        label: string
    }) => {
        if (selectedProduct) {
                        ...
                        <div>
                            <div>
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <p className="text-sm">
                                            Gray ping indicates that the vendor
                                            has{' '}
                                            <span className="font-bold">
                                                Static
                                            </span>{' '}
                                            location broadcast mode. This means
                                            that it is only updated personally
                                            by the vendor and the vendor might
                                            not be exactly on the location
                                            indicated.
                                        </p>
                                        <p className="text-sm mt-2">
                                            ...
                                            below the modal that shows up after
                                            clicking on the vendor marker{')'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <p className="text-sm">
                                        ...
                                        location broadcast mode. In realtime
                                        mode, vendor location is broadcasted
                                        every 5 seconds.
                                    </p>
                                </div>
                            </div>
                        ...
                        >
                            <div id="abcd">
                                ...
                                <section className="mb-8">
                                    ...
                                    {selectedVendor &&
                                        selectedVendor.productsOffered
                                            .length === 0 && (
                                            <p>No available products</p>
                                        )}
                                    <ul className="list-disc pl-4">
                                        {selectedVendor.productsOffered.map(
                                            (product) => (
                                                <li key={product.id}>
                                                    {product.name}
                                                </li>
                                ...
                                <section className="mb-8">
                                    ...
                                    {selectedVendor &&
                                        selectedVendor.featuredImages.length ===
                                            0 && <p>No featured images</p>}
                                    <div className="flex flex-col space-y-4">
                                        {selectedVendor.featuredImages.map(
                                            (image) => (
                                                <div key={image.id}>
                                                    <p>{image.title}</p>
                                                    <img src={image.url} />
                                ...
                                <footer className="mt-2">
                                    ...
                                    {formatDatetime(
                                        selectedVendor.lastLocationUpdate
                                    )}
                                </footer>
                            </div>
                        </BaseModal>
